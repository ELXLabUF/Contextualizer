                        ...
                        this.http.get(response.url!, { responseType: "text" }).subscribe({
                            next: (jsonData) => {
                                ...
                                if (transformedJSONData && "Content Area" in transformedJSONData) {
                                    const result = {"Content Area":transformedJSONData["Content Area"],
                                                    "Grade Level":transformedJSONData["Grade Level"],
                                                    Topic: transformedJSONData["Topic"],
                                                    Duration:transformedJSONData["Duration"],
                                                    CCRSAE: transformedJSONData["CCRSAE"],
                                                    "Instruction Shifts":transformedJSONData["Instruction Shifts"].slice(1,transformedJSONData["Instruction Shifts"].length),
                                                    Objective: this.transformArray(transformedJSONData["Objective"].slice(2,transformedJSONData["Objective"].length)),
                                                    Assessment: this.transformArray(transformedJSONData["Assessment"].slice(2,transformedJSONData["Assessment"].length)),
                                                    Materials: this.transformArray(transformedJSONData["Materials"].slice(1,transformedJSONData["Materials"].length)),
                                                    Instructions: this.transformArray(transformedJSONData["Instructions"].slice(2,transformedJSONData["Instructions"].length)),
                                                    "Home Study": transformedJSONData["Home Study"].slice(1,transformedJSONData["Home Study"].length),
                                                    Reflection: transformedJSONData["Reflection"].slice(2,transformedJSONData["Reflection"].length),
                                        };
                                        resolve(result);
                                    }else {
